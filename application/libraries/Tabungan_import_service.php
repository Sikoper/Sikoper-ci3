...
class Tabungan_import_service
{
	...
	private function _find_nasabah_by_name($nama)
	{
		...
		$result = $this->CI->db->where('LOWER(nama_lengkap)', strtolower(trim($nama)))
			->get('tbnasabah')->row();
	...
	private function _create_nasabah_from_import($nama, $alamat = '-', $telp = '-', $pegawai_id = null)
	{
		...
		$this->CI->db->insert('tbnasabah', $data);
	...
	public function import_saldo_akhir_tahun($file_path, $pegawai_id, $jenistabungan_id, $delete_existing = true)
	{
		...
		if ($delete_existing) {
			...
			$this->CI->db->query("DELETE FROM tbtransaksi WHERE simpanan_id IN (SELECT id FROM tbsimpanan)");
			$results['deleted']['transaksi'] = $this->CI->db->affected_rows();

			// 2. tbdetail_penarikan (references tbsimpanan, tbpenarikan)
			$this->CI->db->query("DELETE FROM tbdetail_penarikan WHERE simpanan_id IN (SELECT id FROM tbsimpanan)");
			$results['deleted']['penarikan'] = $this->CI->db->affected_rows();

			// 3. tbpenarikan (references tbsimpanan)
			$this->CI->db->query("DELETE FROM tbpenarikan WHERE simpanan_id IN (SELECT id FROM tbsimpanan)");

			// 4. tbdetail_simpanan (references tbsimpanan)
			$this->CI->db->query("DELETE FROM tbdetail_simpanan WHERE simpanan_id IN (SELECT id FROM tbsimpanan)");
			$results['deleted']['setoran'] = $this->CI->db->affected_rows();

			// 5. tbsimpanan
			$this->CI->db->query("DELETE FROM tbsimpanan");
			$results['deleted']['simpanan'] = $this->CI->db->affected_rows();

			// 6. tbnasabah (only those without deposito)
			$this->CI->db->query("DELETE FROM tbnasabah WHERE id NOT IN (SELECT DISTINCT nasabah_id FROM tbdeposito)");
			$results['deleted']['nasabah'] = $this->CI->db->affected_rows();
		}
		...
		for ($i = 3; $i < count($des_rows); $i++) {
			...
			try {
				...
				if (!$existing) {
					...
					$simpanan_id = $existing->id;
					$this->CI->db->where('id', $simpanan_id)->update('tbsimpanan', [
						'jumlah_simpanan' => $saldo_akhir,
						'jumlah_bunga' => $bunga
					]);
				}
			...
			try {
				...
				if ($existing) {
					...
					if (!$is_placeholder) {
						...
						$this->CI->db->where('id', $nasabah_id)->update('tbnasabah', [
							'nama_lengkap' => $nama,
							'alamat' => $alamat ?: '-'
						]);
					}
