    ...
    <div class="card">
        <div class="card-body">
                ...
                <div class="col-md-3">
                    ...
                    <select class="form-select" id="bulan" name="bulan">
                        ...
                        foreach ($months as $num => $name) {
                            // This condition decides whether to show the month
                            $is_past_year = $selected_year < $current_year_server;
                            $is_current_year_and_valid_month = ($selected_year == $current_year_server && $num <= $current_month_server);

                            if ($is_past_year || $is_current_year_and_valid_month) {
                                echo '<option value="' . $num . '"' . ($num == $selected_month ? ' selected' : '') . '>' . $name . '</option>';
                            }
                        }
                        ?>
                    </select>
            ...
            <div class="recap-summary text-center my-5">
                ...
                <h1 id="recapMonthYear" class="display-4 text-success fw-bold mb-3">Juli 2025</h1>
                ...
                <div class="card-body">
                    <div class="dataTable-wrapper dataTable-loading no-footer sortable searchable fixed-columns">
                        <div class="dataTable-container">
                            <table id="rekapTable" class="table table-striped dataTable-table" style="width:100%">
                                <thead>
                                    <tr>
...
<script>
    $(document).ready(function() {
        ...
        // Function to load or reload the DataTable
        function load_rekapitulasi(bulan, tahun) {
            // Update titles
            const monthName = $('#bulan option:selected').text();
            $('#recapMonthYear').text(monthName + ' ' + tahun);
            $('#detailTitle').text('Detail Saldo Nasabah - ' + monthName + ' ' + tahun);
            ...
            table = $('#rekapTable').DataTable({
                ...
                "ajax": {
                    ...
                    "dataSrc": function(json) {
                        // Update summary cards with data from the JSON response
                ...
                "columnDefs": [{
                    "targets": [0, 5], // Targets the 'No.' and 'Total Diterima' column
                    "orderable": false,
                }]
            });
        }
        ...
        $('#filterBtn').on('click', function() {
            var bulan = $('#bulan').val();
            var tahun = $('#tahun').val();
            load_rekapitulasi(bulan, tahun);
        });

        // Initial load on page ready
        // Trigger a click to load data for the default selected month/year
