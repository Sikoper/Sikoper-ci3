<div class="card">
    ...
    <div class="card-body">
        ...
        <div id="loading-sheets" class="text-center p-4" style="display:none">
            ...
            <p class="mt-2">Membaca & memproses file Excel... <strong id="sheets-elapsed">00:00</strong></p>
            <small class="text-muted" id="sheets-tip">File besar (30MB+) membutuhkan waktu 5-15 menit untuk
                diproses</small>
        </div>
        ...
        <div id="preview-loading" class="text-center p-4" style="display:none">
            <div class="spinner-border text-primary" role="status"></div>
            <p class="mt-2">Membaca data sheet... <strong id="preview-elapsed">00:00</strong></p>
            <small class="text-muted" id="preview-tip">File besar (30MB+) bisa memakan waktu 1-3 menit</small>
        </div>
        ...
        <div id="preview-section" class="mt-3" style="display:none">
            ...
            <div class="card bg-light mb-3">
                <div class="card-body">
                    <h6 class="card-title"><i class="fa fa-cogs"></i> Mapping Kolom <span
                            class="badge bg-info">Auto-refresh</span></h6>
            ...
            <div class="mt-3">
                ...
                <button type="button" class="btn btn-secondary btn-lg" id="btn-import-loading" style="display:none"
                    disabled>
            ...
            <div class="card border-primary">
                <div class="card-body">
                    <div class="d-flex align-items-center mb-3">
                        ...
                        <div>
                            <h5 class="mb-0" id="import-status-text">Memproses import data...</h5>
                            <small class="text-muted" id="import-substatus">Mengirim data ke server...</small>
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-between text-muted small">
                        <span><i class="fa fa-clock-o"></i> Waktu berjalan: <strong
                                id="elapsed-time">00:00</strong></span>
                        <span id="import-tip" class="text-info">File besar bisa memakan waktu 2-5 menit</span>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    $(document).ready(function () {
        ...
        $('#form-upload').on('submit', function (e) {
            ...
            $('#sheets-elapsed').text('00:00');
            $('#sheets-tip').text('Mengupload file dan memulai parsing...');

            var sheetsStart = Date.now();
            var sheetsTimer = setInterval(function () {
                var el = Math.floor((Date.now() - sheetsStart) / 1000);
                $('#sheets-elapsed').text(String(Math.floor(el / 60)).padStart(2, '0') + ':' + String(el % 60).padStart(2, '0'));
            }, 1000);

            // Upload file - server starts background parsing and returns immediately
            $.ajax({
                ...
                success: function (response) {
                    if (response.success && response.parsing) {
                        // File uploaded, background parsing started
                        $('#sheets-tip').text('File diupload! Memproses Excel di background...');

                        // Start polling for parse completion
                        var pollBusy = false;
                        var pollInterval = setInterval(function () {
                            if (pollBusy) return; // Skip if previous poll still running (inline fallback)
                            pollBusy = true;
                            $.ajax({
                                url: '<?= base_url("simpanan/check_parse_status") ?>',
                                type: 'GET',
                                dataType: 'json',
                                timeout: 600000, // 10 min timeout for inline fallback
                                success: function (status) {
                                    pollBusy = false;
                                    if (status.status === 'done' && status.sheets) {
                                        // DONE! Show sheets
                                        ...
                                        $('#btn-read-sheets').prop('disabled', false);
                                        $('#loading-sheets').hide();
                                        alert('Error: ' + (status.error || 'Gagal parsing'));
                                    } else {
                                        // Still processing - update tip
                                        $('#sheets-tip').text(status.message || 'Memproses...');
                                    }
                                },
                                error: function () {
                                    pollBusy = false;
                    ...
                    } else if (response.success && response.sheets) {
                        // Legacy: direct response with sheets
                        clearInterval(sheetsTimer);
                        $('#btn-read-sheets').prop('disabled', false);
                        $('#loading-sheets').hide();
                        var options = '';
        ...
        $('#btn-preview').on('click', function () {
            ...
            $('#preview-section').hide();
            $('#preview-elapsed').text('00:00');
            $('#preview-tip').text('File besar (30MB+) bisa memakan waktu 1-3 menit');

            var previewStart = Date.now();
            var previewTimer = setInterval(function () {
                var el = Math.floor((Date.now() - previewStart) / 1000);
                $('#preview-elapsed').text(String(Math.floor(el / 60)).padStart(2, '0') + ':' + String(el % 60).padStart(2, '0'));
                if (el > 60) $('#preview-tip').text('Masih membaca... file besar membutuhkan waktu lebih lama');
            }, 1000);
            ...
            $.ajax({
                ...
                error: function (xhr, status, error) {
                    ...
                    $('#preview-loading').hide();
                    if (status === 'timeout') {
                        alert('Timeout: Preview memakan waktu terlalu lama. Coba refresh dan pilih sheet lagi.');
                    } else {
                        alert('Error: ' + error);
                    }
                }
