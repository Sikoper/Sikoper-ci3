...
<head>
    ...
    <style>
        /* CSS yang sudah dioptimalkan untuk Dompdf */
        @page { size: A4 landscape; margin: 1cm; }
        body { font-family: 'Times New Roman', Times, serif; font-size: 11pt; margin: 0; padding: 0; }
        
        <?php if ($halaman_dicetak == 'depan') { echo '#halaman-belakang { display: none; }'; } ?>
        <?php if ($halaman_dicetak == 'belakang') { echo '#halaman-depan { display: none; }'; } ?>

        #halaman-depan { page-break-after: always; } /* Memastikan halaman belakang ada di halaman baru */
        
        .page-container { width: 100%; height: 100%; position: relative; }
        .kop { text-align: center; line-height: 1.4; }
        .kop h2 { font-size: 15pt; margin: 0; font-weight: bold; }
        .kop h3 { font-size: 14pt; margin: 0; font-weight: bold; }
        .kop p { font-size: 11pt; margin: 0; }
        .nomor-seri { position: absolute; top: 0; right: 0; font-size: 11pt; }
        
        .isi-utama { margin-top: 1cm; }
        .isi-utama table { width: 100%; border-collapse: collapse; }
        .isi-utama td { vertical-align: top; padding: 2px 0; }
        
        .kwitansi { margin-top: 20px; border-top: 1.5px dashed #000; padding-top: 8px; }
        
        .tanda-tangan-area { text-align: right; margin-top: 20px; }
        .tanda-tangan-box { display: inline-block; text-align: center; width: 45%; }
        .tanda-tangan-box .jabatan, .tanda-tangan-box .tanggal { margin: 0; padding: 0; font-size: 11pt; }
        .tanda-tangan-box .nama { font-weight: bold; text-decoration: underline; margin-top: 60px; }
        
        .layout-dua-kolom { display: table; width: 100%; table-layout: fixed; border-spacing: 20px 0; }
        .kolom { display: table-cell; vertical-align: top; }
        
        .ketentuan ol { padding-left: 20px; font-size: 9pt; line-height: 1.4; margin-top:0; }
        .ketentuan li { margin-bottom: 1.5px; }
        
        .surat-pernyataan-box { border: 1px solid #555; padding: 10px; font-size: 10pt; }
        .surat-pernyataan-box table td { padding: 1.5px 0; }
        
        .tanda-tangan-nasabah { text-align: center; float:right; margin-top:10px; }
        .tanda-tangan-nasabah p { margin:0; padding:0; font-size: 10pt; }
        .tanda-tangan-nasabah .nama { font-weight: bold; text-decoration: underline; margin-top: 10px; }
        
        .materai-box { border: 1.5px solid #333; width: 100px; height: 35px; text-align: center; padding-top: 15px; box-sizing: border-box; font-size: 8pt; color: #333; margin: 5px auto; font-weight: bold; }
        
        .tabel-bunga { font-size: 9pt; width: 100%; border-collapse: collapse; margin-top: 10px; }
        .tabel-bunga th, .tabel-bunga td { border: 1px solid black; text-align: center; padding: 4px; height: 16px;}
    </style>
</head>
<body>
    <div id="halaman-depan" class="page-container">
        ...
        <div class="isi-utama">
            <table style="width: 80%;">
                <tr><td style="width: 20%;">Telah terima dari</td><td style="width: 2%;">:</td><td style="width: 78%; font-weight: bold;"><?= $nama_nasabah ?></td></tr>
                <tr><td>Alamat Nasabah</td><td>:</td><td><?= $alamat_nasabah ?></td></tr>
                <tr><td>Uang Sejumlah</td><td>:</td><td style="font-weight: bold;">Rp. <?= number_format($jumlah_deposito, 2, ',', '.') ?></td></tr>
                <tr><td>Dengan Huruf</td><td>:</td><td style="font-style: italic;"><?= $terbilang ?></td></tr>
            </table>
            <p style="margin-top: 15px; line-height: 1.6;">Untuk Simpanan Berjangka dalam waktu <?= $durasi ?> bulan, mulai tanggal <?= date('d F Y', strtotime($tanggal_deposito)) ?> sampai dengan tanggal <?= date('d F Y', strtotime($tanggal_jatuh_tempo)) ?>, bunga <?= $suku_bunga ?>% perbulan, dengan syarat-syarat yang telah ditentukan oleh Usaha Simpan Pinjam Bali Sejahtera Desa Adat Culik.</p>
        </div>
        <div class="kwitansi">
        ...
        <div class="layout-dua-kolom">
            ...
            <div class="kolom">
                <div class="surat-pernyataan-box">
                    <h3 style="text-align: center; text-decoration: underline; font-weight: bold; margin-top:0;">SURAT PERNYATAAN</h3><p>Yang bertandatangan di bawah ini:</p>
                    <table>
                        <tr><td style="width: 35%;">Nama</td><td style="width: 5%;">:</td><td style="font-weight: bold;"><?= $nama_nasabah ?></td></tr>
                        <tr><td>NIK</td><td>:</td><td><?= $nik_nasabah ?></td></tr>
                        <tr><td>Tempat Tanggal Lahir</td><td>:</td><td><?= $tempat_lahir ?>, <?= date('d F Y', strtotime($tanggal_lahir)) ?></td></tr>
                        <tr><td>Alamat</td><td>:</td><td><?= $alamat_nasabah ?></td></tr>
                        <tr><td>No. Telp</td><td>:</td><td><?= $telp_nasabah ?></td></tr>
                    </table>
                    ...
                    <div class="tanda-tangan-nasabah"><p>Culik, <?= date('d F Y', strtotime($tanggal_deposito)) ?></p><p>Yang tersebut di atas</p><div class="materai-box">MATERAI TEMPEL</div><p class="nama"><?= $nama_nasabah ?></p></div>
                </div>
            </div>
        </div>
        <div class="layout-dua-kolom">
            <div class="kolom"><table class="tabel-bunga"><tr><th>No</th><th>Tgl</th><th>Bunga</th><th>No.Bukti</th><th>Paraf</th></tr><?php for ($i=1; $i<=6; $i++) echo "<tr><td>$i</td><td></td><td></td><td></td><td></td></tr>"; ?></table></div>
            <div class="kolom"><table class="tabel-bunga"><tr><th>No</th><th>Tgl</th><th>Bunga</th><th>No.Bukti</th><th>Paraf</th></tr><?php for ($i=7; $i<=12; $i++) echo "<tr><td>$i</td><td></td><td></td><td></td><td></td></tr>"; ?></table></div>
        </div>
    </div>
</body>
</html>