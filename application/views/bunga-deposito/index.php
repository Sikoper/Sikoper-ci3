<div id="loadingOverlay" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(255,255,255,0.7); z-index:9999;">
    <div style="position:absolute; top:50%; left:50%; transform:translate(-50%, -50%); text-align:center;">
        ...
        <p style="margin-top: 1rem; color:#333; font-weight:bold;">Memproses...</p>
    </div>
</div>
            ...
            <form id="formTambahBunga">
                <div class="modal-body">
                    <div class="form-group mb-3">
                        <label for="deposito_id">Pilih Deposito</label>
                        <select id="deposito_id" name="deposito_id" class="form-control" style="width:100%"></select>
                        <small class="text-muted">Ketik nama nasabah atau nomor rekening</small>
                    </div>
                    
                    <div id="infoDeposito" class="mb-3" style="display:none;">
                    ...
                    <div class="form-group mb-3">
                        <label for="tanggal_perhitungan">Tanggal Perhitungan</label>
                        <input type="date" id="tanggal_perhitungan" name="tanggal_perhitungan" class="form-control" value="<?= date('Y-m-d') ?>">
                        <small class="text-muted">Hanya 1 bunga per deposito per bulan</small>
                    </div>
...
<link href="<?= base_url('assets') ?>/vendors/select2-bootstrap-5-theme/select2-bootstrap-5-theme.min.css" rel="stylesheet" />
<!-- Select2 JS -->
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#deposito_id').select2({
            ...
            ajax: {
                url: '<?= site_url('bunga_deposito/search_deposito') ?>',
                dataType: 'json',
                delay: 250,
                data: function(params) {
                    return { 
                        q: params.term,
                        target_date: $('#tanggal_perhitungan').val()
                    };
                },
        ...
        $('#tanggal_perhitungan').on('change', function() {
            $('#deposito_id').val(null).trigger('change');
            $('#infoDeposito').hide();
            $('#btnSimpanBunga').prop('disabled', true);
            currentBunga = 0;
        });

        // Show info when deposito selected
        $('#deposito_id').on('select2:select', function(e) {
            const data = e.params.data;
            $('#infoDeposito').show();
            $('#infoNasabah').text(data.nama_nasabah);
            $('#infoRate').text(data.rate_bunga);
            $('#infoJumlah').text(new Intl.NumberFormat('id-ID').format(data.jumlah_deposito));
            
            currentBunga = parseFloat(data.jumlah_deposito) * (parseFloat(data.rate_bunga) / 100);
        ...
        $('#deposito_id').on('select2:clear', function() {
            $('#infoDeposito').hide();
            currentBunga = 0;
            $('#jumlah_bunga').val(0);
            $('#btnSimpanBunga').prop('disabled', true);
        ...
        $('#formTambahBunga').submit(function(e) {
            ...
            if (!$('#deposito_id').val() || !$('#tanggal_perhitungan').val()) {
                Swal.fire('Peringatan', 'Pilih deposito dan tanggal.', 'warning');
                return;
            }

            $.ajax({
                url: '<?= site_url('bunga_deposito/simpan_bunga_manual') ?>',
                type: 'POST',
                data: {
                    deposito_id: $('#deposito_id').val(),
                    tanggal_perhitungan: $('#tanggal_perhitungan').val(),
        ...
        $('#btnPembungaanDeposito').click(function() {
            Swal.fire({
                ...
                html: "Proses ini akan menghitung bunga untuk semua deposito aktif.<br><b>Tidak dapat dibatalkan.</b>",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        complete: function() {
                            $('#btnPembungaanDeposito').prop('disabled', false).html('<i class="fa fa-calculator"></i> Hitung Bunga Hari Ini');
    ...
    function printLaporan() {
        ...
        Swal.fire({
            title: "Cetak Laporan?",
            html: "Cetak laporan bunga deposito periode <strong>" + startDate + "</strong> s/d <strong>" + endDate + "</strong>?",
            icon: "question",
            showCancelButton: true,
            confirmButtonColor: "#28a745",
            ...
            if (result.isConfirmed) {
                window.open("<?= base_url('bunga_deposito/print_laporan_bulanan') ?>?start_date=" + startDate + "&end_date=" + endDate, "_blank");
            }
        });
    }
</script>